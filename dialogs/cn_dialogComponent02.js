...
const CN_DIALOG_COMPONENT02 = 'CN_DIALOG_COMPONENT02';
...
class CN_DialogComponent02 extends ComponentDialog {
    constructor(logger) {
        super(CN_DIALOG_COMPONENT02);
        this.logger = logger;
    ...
    async queryDatabaseStep(stepContext) {
        var adapter = new FileSync(path.resolve(__dirname, "../db/" + Database.Component02.db));
        var lowdb = low(adapter);
        return await stepContext.prompt(PROMPT_CHOICE_COMPONENT, {
            prompt: Hint.Component02_SelectComponent,
    ...
    async queryDisplayStep(stepContext) {
        ...
        var template = new ACData.Template(Card.Component02_AdaptiveComponent);
    ...
    async queryConfirmationStep(stepContext) {
        ...
        if (stepContext.result.index === 0) {
            ...
            return await stepContext.prompt(PROMPT_TEXT_QUESTION, {
                prompt: Hint.Component02_promptQuestion,
                retryPrompt: Hint.Component02_retryQuestion,
            });
        }
    }
...
module.exports.CN_DialogComponent02 = CN_DialogComponent02;
module.exports.CN_DIALOG_COMPONENT02 = CN_DIALOG_COMPONENT02;
